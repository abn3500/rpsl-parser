...
public class AttributeLexerWrapper {
	...
	public List<Pair<String, List<String>>> parse(Reader in) throws IOException {
		...
		while(lexerState > 2 && lexerState != 256) { //256 is parser error code, [-1,2] are parser error (including EOF)
			...
			if (!stateTable.containsKey(lexerState)) {
				System.err.println("Unknown state " + lexerState + ", text was: " + lexer.yytext());
			...
			if(stateName.startsWith("KEYW_")) {
				...
				//append the token, will skip any tokens parsed before the first keyword
				if(previousKeyWord != null && lexer.yylength() > 0) {
	 ...
	 */
	public static List<Pair<String, List<String>>> parse(RpslAttribute attr) {
		try {
			...
			System.err.println("No lexer available for " + attr.getType().getName());
		} catch (IOException e) {
