...
public class BGPPeerTest {
	...
	public void checksSyntaxOfAS() {
		(new BGPPeer("NOTANAS", "2.2.2.2", inetRtr)).toString();
	}
	
	@Test(expected=IllegalArgumentException.class)
	public void checkSyntaxOfAddress() {
		(new BGPPeer("AS2", "2.2.2.2.2.2.2.2", inetRtr)).toString();
	}
	
	@Test
	public void checkToString() {
		assertEquals("AS2(2.2.2.2)-peer-of-TEST-AS(1.1.1.1)", (new BGPPeer("AS2", "2.2.2.2", inetRtr)).toString());
	}
	
	@Test
	public void checkAddRouteTable() {
		BGPPeer bgpPeerOne = new BGPPeer("AS2", "2.2.2.2", inetRtr),
				bgpPeerTwo = new BGPPeer("AS2", "2.2.2.3", inetRtr);
		...
		BGPRouteTable table = new BGPRouteTable("AS2", "2.2.2.2", bgpPeerOne.speaker.speakerName, routeSet);
	...
	public void checkEquality() {
		assertEquals("Same peers should be equal", (new BGPPeer("AS2", "2.2.2.2", inetRtr)), (new BGPPeer("AS2", "2.2.2.2", inetRtr)));
		assertNotEquals("Different peers should not be equal", (new BGPPeer("AS2", "2.2.2.2", inetRtr)), (new BGPPeer("AS2", "2.2.2.3", inetRtr)));
	}
